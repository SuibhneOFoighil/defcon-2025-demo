...
interface UseTemplateLogsOptions {
  userID?: string
  tail?: number
  cursor?: number
  enabled?: boolean
...
export function useTemplateLogs(options: UseTemplateLogsOptions = {}) {
  ...
  const queryResult = useQuery({
    queryKey: ["templateLogs", options.userID, options.tail, options.cursor],
    queryFn: async () => {
      ...
      const response = await fetch(`/api/ludus/templates/logs?${searchParams.toString()}`)
      
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ error: 'Unknown error' }))
        throw new Error(`Failed to fetch template logs: ${errorData.error || response.statusText}`)
      }
